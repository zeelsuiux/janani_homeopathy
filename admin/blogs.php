...
<?php if ($canEdit): ?><div class="blog-modal" id="blogModal" aria-hidden="true"><div class="blog-modal-dialog" role="dialog" aria-modal="true" aria-labelledby="blogModalTitle"><div class="blog-modal-header"><h2 id="blogModalTitle">Add New Blog</h2><button class="blog-modal-close" type="button" id="closeBlogModal" aria-label="Close">&times;</button></div><form method="post" enctype="multipart/form-data"><div class="form-grid"><div class="field full"><label>Blog Title</label><input required name="title"></div><div class="field"><label>Featured Image</label><input type="file" name="image" accept="image/*"></div><div class="field full"><label>Content</label><div class="toolbar"><button type="button" onclick="cmd('bold')"><b>B</b></button><button type="button" onclick="cmd('italic')"><i>I</i></button><button type="button" onclick="cmd('underline')"><u>U</u></button><span class="font-size-options" role="radiogroup" aria-label="Font size"><button type="button" class="font-size-button" aria-label="Small" aria-pressed="false" onmousedown="event.preventDefault()" onclick="setFontSize('1', this)">A<small>S</small></button><button type="button" class="font-size-button active" aria-label="Normal" aria-pressed="true" onmousedown="event.preventDefault()" onclick="setFontSize('3', this)">A</button><button type="button" class="font-size-button" aria-label="Large" aria-pressed="false" onmousedown="event.preventDefault()" onclick="setFontSize('5', this)">A<small>L</small></button><button type="button" class="font-size-button" aria-label="Extra large" aria-pressed="false" onmousedown="event.preventDefault()" onclick="setFontSize('7', this)">A<small>XL</small></button></span></div><div id="editor" class="rich" contenteditable="true"></div><textarea hidden name="content" id="content"></textarea></div><div class="field full"><button class="btn" type="submit" onclick="document.getElementById('content').value=document.getElementById('editor').innerHTML">Publish Blog</button></div></div></form></div></div><?php endif; ?>
<div class="table-wrap"><table id="blogsTable"><tr><th>Image</th><th>Title</th><th>Date</th><th>Action</th></tr><?php foreach (array_reverse($db['blogs']) as $b): ?><tr class="blog-row" data-search="<?=e(strtolower(($b['title']??'').' '.($b['content']??'')))?>"><td><?php if ($b['image']): ?><img src="../<?=e($b['image'])?>" style="width:70px;height:45px;object-fit:cover;border-radius:6px"><?php endif; ?></td><td><?=e($b['title'])?></td><td><?=e(date_fmt($b['created_at']))?></td><td><?php if ($canEdit): ?><a class="btn btn-sm btn-light" href="blog-edit.php?id=<?=e($b['id'])?>">Edit</a><?php endif; ?> <?php if ($canDelete): ?><a class="btn btn-sm btn-danger" href="blog-delete.php?id=<?=e($b['id'])?>" onclick="return confirm('Delete blog?')">Delete</a><?php elseif (!$canEdit): ?>- <?php endif; ?></td></tr><?php endforeach; ?><tr id="noBlogsFound" style="display:none"><td colspan="4" style="text-align:center;padding:30px">No blogs found.</td></tr></table></div>
<script>
function cmd(command, value) { document.execCommand(command, false, value || null); }
function setFontSize(value, button) { document.execCommand('fontSize', false, value); document.querySelectorAll('.font-size-button').forEach(function (item) { item.classList.remove('active'); item.setAttribute('aria-pressed', 'false'); }); button.classList.add('active'); button.setAttribute('aria-pressed', 'true'); }
